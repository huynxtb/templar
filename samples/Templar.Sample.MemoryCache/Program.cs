...
// The default cache, made visible. MemoryTemplateCache holds one entry per template key for
// Templates:CacheSeconds — 30 here, short enough to watch it expire — and the store behind it counts
// the reads that reach it. Render the same template twice and /api/cache/stats does not move; save,
// delete or clear, and the next read is a miss again.
//
// Everything this sample does is in this one file, top to bottom.
//
//   dotnet run                         → http://localhost:5001/swagger
...
builder.Services.AddTemplar(options =>
{
...
app.MapOpenApi();
app.UseSwaggerUI(options => options.SwaggerEndpoint("/openapi/v1.json", "Templar · in-process cache"));
