...
// Templar on MongoDB. Everything this sample does is in this one file, top to bottom: configuration,
// registration, seeding, then the HTTP API. Browse and call it at /swagger.
//
//   make up SAMPLE=Mongo               start a MongoDB container
//   dotnet run                         → http://localhost:5014/swagger
//   dotnet run --Templates:ConnectionString="mongodb://localhost:27017"
...
if (string.IsNullOrWhiteSpace(connectionString))
    throw new InvalidOperationException(
        ...
        + "--Templates:ConnectionString=\"mongodb://localhost:27017\" on the command line.");

// AddTemplar registers the three services, the Mustache engine and the cache; UseMongo supplies the
// store behind them. The only non-relational store: the same fields camelCased, with the three key
// parts inside a composite _id. Template keys match byte-wise, so there is no schema to configure.
builder.Services
    .AddTemplar(options =>
    {
    ...
    {
        store.DatabaseName = settings["Database"] ?? store.DatabaseName;
        store.CollectionName = settings["TableName"] ?? store.CollectionName;
    });

var app = builder.Build();

app.MapOpenApi();
app.UseSwaggerUI(options => options.SwaggerEndpoint("/openapi/v1.json", "Templar · MongoDB"));
