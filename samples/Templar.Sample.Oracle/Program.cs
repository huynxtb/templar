...
// Templar on Oracle Database. Everything this sample does is in this one file, top to bottom:
// configuration, registration, seeding, then the HTTP API. Browse and call it at /swagger.
//
//   make up SAMPLE=Oracle              start an Oracle Free container with these credentials
//   dotnet run                         → http://localhost:5013/swagger
//   dotnet run --Templates:ConnectionString="User Id=notify;Password=secret;Data Source=…"
...
if (string.IsNullOrWhiteSpace(connectionString))
    throw new InvalidOperationException(
        ...
        + "--Templates:ConnectionString=\"User Id=notify;Password=secret;Data Source=localhost:1521/FREEPDB1\" on the command line.");

// AddTemplar registers the three services, the Mustache engine and the cache;
// UseOracle supplies the store behind them. Upserts use MERGE, and the driver is told to bind by
// name because that MERGE mentions its parameters out of order.
builder.Services
    .AddTemplar(options =>
    {
    ...
    .UseOracle(connectionString, store =>
    {
        ...
        // Identifiers are upper-cased before quoting, which is how conventional Oracle DDL stores
        // them. Set this when the table was created with quoted lower-case names.
        store.PreserveIdentifierCase = settings.GetValue("PreserveIdentifierCase", false);
    });

var app = builder.Build();

app.MapOpenApi();
app.UseSwaggerUI(options => options.SwaggerEndpoint("/openapi/v1.json", "Templar · Oracle"));
