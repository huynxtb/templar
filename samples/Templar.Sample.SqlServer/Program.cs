...
// Templar on SQL Server / Azure SQL. Everything this sample does is in this one file, top to bottom:
// configuration, registration, seeding, then the HTTP API. Browse and call it at /swagger.
//
//   make up SAMPLE=SqlServer           start a SQL Server container and create the database
//   dotnet run                         → http://localhost:5012/swagger
//   dotnet run --Templates:ConnectionString="Server=…;Database=…;User ID=sa;Password=…"
...
if (string.IsNullOrWhiteSpace(connectionString))
    throw new InvalidOperationException(
        ...
        + "--Templates:ConnectionString=\"Server=localhost,1433;Database=notifications;User ID=sa;Password=…;TrustServerCertificate=true\" on the command line.");

// AddTemplar registers the three services, the Mustache engine and the cache;
// UseSqlServer supplies the store behind them. Upserts use MERGE … WITH (HOLDLOCK).
builder.Services
    .AddTemplar(options =>
    {
    ...
    .UseSqlServer(connectionString, store =>
    {
        ...
        store.Schema = settings["Schema"] ?? store.Schema;
...
app.MapOpenApi();
app.UseSwaggerUI(options => options.SwaggerEndpoint("/openapi/v1.json", "Templar · SQL Server"));
