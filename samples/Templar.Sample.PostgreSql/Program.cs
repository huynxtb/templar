...
// Templar on PostgreSQL. Everything this sample does is in this one file, top to bottom:
// configuration, registration, seeding, then the HTTP API. Browse and call it at /swagger.
//
//   make up SAMPLE=PostgreSql          start a PostgreSQL container with these credentials
//   dotnet run                         → http://localhost:5010/swagger
//   dotnet run --Templates:ConnectionString="Host=…;Database=…;Username=…;Password=…"
...
if (string.IsNullOrWhiteSpace(connectionString))
    throw new InvalidOperationException(
        ...
        + "--Templates:ConnectionString=\"Host=localhost;Database=notifications;…\" on the command line.");

// AddTemplar registers the three services, the Mustache engine and the cache;
// UsePostgreSql supplies the store behind them. Upserts use ON CONFLICT.
builder.Services
    .AddTemplar(options =>
    {
    ...
    .UsePostgreSql(connectionString, store =>
    {
...
app.MapOpenApi();
app.UseSwaggerUI(options => options.SwaggerEndpoint("/openapi/v1.json", "Templar · PostgreSQL"));
